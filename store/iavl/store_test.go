...
func newAlohaTree(t *testing.T, db dbm.DB) (*iavl.MutableTree, types.CommitID) {
	...
	tree := iavl.NewMutableTree(dbt, cacheSize, false, log.NewTestLogger(t))
...
func TestLoadStore(t *testing.T) {
	...
	newHStore, err := LoadStore(db, cmtlog.NewNopLogger(), types.NewKVStoreKey("test"), cIDH, DefaultIAVLCacheSize, false, false)
	require.NoError(t, err)
	require.Equal(t, string(newHStore.Get([]byte("hello"))), "hallo")

	// Querying a new store at some previous pruned height Hp
	newHpStore, err := LoadStore(db, cmtlog.NewNopLogger(), types.NewKVStoreKey("test"), cIDHp, DefaultIAVLCacheSize, false, false)
	require.NoError(t, err)
	require.Equal(t, string(newHpStore.Get([]byte("hello"))), "hola")

	// Querying a new store at current height H
	newHcStore, err := LoadStore(db, cmtlog.NewNopLogger(), types.NewKVStoreKey("test"), cIDHc, DefaultIAVLCacheSize, false, false)
	require.NoError(t, err)
...
func TestIAVLReverseIterator(t *testing.T) {
	...
	tree := iavl.NewMutableTree(db, cacheSize, false, log.NewTestLogger(t))
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(db *dbm.MemDB) *Store {
				tree := iavl.NewMutableTree(wrapper.NewCosmosDB(db), cacheSize, false, log.NewNopLogger())
				store := UnsafeNewStore(tree)
